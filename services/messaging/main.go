...
func initDB(db *sql.DB) error {
	...
	// Ensure a default public channel exists for first-run usability.
	if err := ensureDefaultChannel(db, "general", "General", "public"); err != nil {
		return err
	}
	return nil
}
...
func hasColumn(q sqlExecQuerier, table, column string) bool {
	var count int
	_ = q.QueryRow(fmt.Sprintf("SELECT COUNT(*) FROM pragma_table_info('%s') WHERE name = ?", table), column).Scan(&count)
	return count > 0
}

func ensureDefaultChannel(db *sql.DB, id, name, chType string) error {
	var exists bool
	if err := db.QueryRow("SELECT EXISTS(SELECT 1 FROM channels WHERE id = ?)", id).Scan(&exists); err != nil {
		return err
	}
	if exists {
		return nil
	}
	return insertChannel(db, id, name, chType)
}

func insertChannel(q sqlExecQuerier, id, name, chType string) error {
	// Support both the lightweight messaging schema and the richer admin schema.
	now := time.Now().Unix()
	if hasColumn(q, "channels", "created_at") && hasColumn(q, "channels", "updated_at") {
		_, err := q.Exec(
			"INSERT INTO channels (id, name, type, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
			id, name, chType, now, now,
		)
		return err
	}
	_, err := q.Exec("INSERT INTO channels (id, name, type) VALUES (?, ?, ?)", id, name, chType)
	return err
}

func insertChannelMember(q sqlExecQuerier, channelID, userID string) error {
	now := time.Now().Unix()
	if hasColumn(q, "channel_members", "joined_at") {
		// Admin schema: joined_at is NOT NULL; role defaults to 'member'.
		if hasColumn(q, "channel_members", "role") {
			_, err := q.Exec(
				"INSERT INTO channel_members (channel_id, user_id, role, joined_at) VALUES (?, ?, ?, ?)",
				channelID, userID, "member", now,
			)
			return err
		}
		_, err := q.Exec(
	...
	_, err := q.Exec("INSERT INTO channel_members (channel_id, user_id) VALUES (?, ?)", channelID, userID)
	return err
}
...
func (r *MessageRouter) findOrCreateDMChannel(u1, u2 string) (string, error) {
	...
	if !exists {
		...
		// Create DM channel; compatible with admin schema.
		if err := insertChannel(tx, dmID, "Direct Message", "dm"); err != nil {
			return "", err
		}
		if err := insertChannelMember(tx, dmID, u1); err != nil {
			return "", err
		}
		if err := insertChannelMember(tx, dmID, u2); err != nil {
			return "", err
		}
