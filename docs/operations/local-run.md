...
# or: docker-compose -f backend/deploy/docker-compose.local.yml build

# Start all services in the background
make -f backend/deploy/Makefile run-all
# or:
cd backend/deploy && make run-all
# or: docker-compose -f backend/deploy/docker-compose.local.yml up -d

# View logs (optional)
make -f backend/deploy/Makefile logs
# or:
cd backend/deploy && make logs
# or: docker-compose -f backend/deploy/docker-compose.local.yml logs -f
```
...
# Send a message (base64 content example; requires JWT user token)
...
# or: docker-compose -f backend/deploy/docker-compose.local.yml down
```
...
- **Messaging unauthorized**: Pastikan request membawa `Authorization: Bearer <token>` atau untuk WS pakai `?token=<token>`.
- **Docker build fails**: Ensure Docker context is the repo root so `backend/pkg/` and `backend/services/` are available; use `docker-compose -f backend/deploy/docker-compose.local.yml build` from the root.
