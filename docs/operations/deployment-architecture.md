...
- **Docker**: Each service can run as a container; `docker-compose` for single-node or small cluster (Portainer stack: `backend/deploy/docker-compose.yml`, local build stack: `backend/deploy/docker-compose.local.yml`).
